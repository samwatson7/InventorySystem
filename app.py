...
def is_csv_ready(): 
    ...
    if not os.path.exists(csv_path) or os.path.getsize(csv_path) == 0:              # if Statement   => Checks if CSV file doesn't exist or is empty
        with open(csv_path, mode="w", encoding="utf-8", newline="") as file:        # with statement => opens CSV file in write mode (creating CSV if it doesn't exist) 
            csv.writer(file).writerow(column_header)                                # Writes header row (Column names) to CSV
